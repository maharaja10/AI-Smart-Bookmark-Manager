...
export async function PATCH(
  ...
  try {
    ...
    const { isReadLater } = await req.json();
    ...
    // Capture snapshot before read later change
    ...
    try {
      ...
      const actionType = isReadLater ? "readlater_added" : "readlater_removed";
    ...
    console.error("Error updating bookmark read later status:", error);
    return NextResponse.json(
      { message: "Error updating bookmark read later status" },
      { status: 500 }
    );
  }
} 